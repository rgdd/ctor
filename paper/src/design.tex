...
it can be successfully resolved to a specific STH.\footnote{%
	The fetched inclusion proof should be fetched against the first known STH
	that must have incorporated the certificate in question (a single history of
	STHs is available in the Tor consensus):
		the mis-issued certificate might have been merged into the log
		reactively despite there being an MMD violation (upon learning that a
			CTR reported the SFO),
	such that a valid inclusion proof can be returned with regards to a more
	recent STH but not earlier ones that captured the log's misbehavior.
