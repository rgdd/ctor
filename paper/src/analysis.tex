...
The CTR criteria set in Section~\ref{sec:base:consensus:ctr-flag} matches over
...
% Removing this below since it involves changing the design:
% Another option that avoids early signals to the
% logs would be to toggle a flag in the Tor consensus, such that newly issued SFOs
% are reported to the auditors directly (i.e., without a failed query).  What is
% most appropriate is circumstantial and depends on which trade-offs one is
% willing to accept. Therefore, we leave it at examples.
