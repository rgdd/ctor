...
where SFOs are presented to TB and submitted probabilistically to CTRs,
...
\begin{figure*}
	...
	\caption{%
		...
		SFO to TB, and TB in turn with some probability
...
The existing \texttt{known-flags} item determines the different flags that the
consensus might contain.  We add another flag named \texttt{CTR}, which
indicates that a Tor relay should support CT-auditing as described later. For
now, assume that a relay qualifies as a CTR if it is flagged as \texttt{stable}
and not \texttt{exit}, to spare the relatively sparse exit bandwidth and only use
relays that can be expected to stay online. Section~\ref{sec:privacy} discusses
trade-offs in the assignment of the \texttt{CTR} flag.
...
which means that a majority of directory authorities voted for inclusion by
proposing a \texttt{ct-log-info} entry containing a log ID, a public key, and a
base URL~\cite{ct,ct/bis}.  While it is a natural starting point to recognize
the same logs as TB, such overlap is not required because the base
design only adds \emph{certificate chains} in other independent logs that accept
them.  This simplifies the addition of community-driven logs that, for example,
could serve the primary purpose of facilitating auditing and increasing
diversity to distribute mis-issued certificates widely.  The premise of
trust anchor policies that cover the commonly accepted root certificates is
aligned with standardized suggestions~\cite{ct,ct/bis} as well as Apple's
requirements~\cite{apple-log-policy}.  We vet this assumption further based on
related work in Appendix~\ref{app:ct-trust-anchors}~\cite{ct-root-landscape}.
...
The first phase covers TB and how an SFO is validated in the TLS
handshake.  We rely on TB's trust store to check certificate chains, an
...
\begin{enumerate}
	...
	\item Raise a certificate transparency error and stop if the SCTs of $s$
		fail TB's SCT-centric CT policy.
...
Each CTR keeps a least recently used cache of the SFOs it validates, e.g., by
hashing the entire SFO or selected SCTs in it. A few megabytes should
significantly reduce load on CT logs. Also note that the steps above are done in
parallel for each CT log with a \texttt{ct-log-info} entry in the consensus.
This assures that the unavailability of one CT log does not halt auditing of
others.
