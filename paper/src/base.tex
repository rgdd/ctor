...
a base URL~\cite{ct,ct/bis}. Note that there does not need to be any overlap
between the logs that the Tor consensus recognizes and those in Tor Browser's CT
policy in this design (later extensions change this). Overlap is likely though.
The ideal scenario is that there are many independent logs that accept
certificate chains for most trust anchors, such that potentially mis-issued
certificates can be widely spread.  TODO Rasmus: wrap-up here and cite the paper
that talked about this, maybe also short LE note where this concern comes from?
Yep, briefly and then refer to Appendix~\ref{app:ct-trust-anchors}.
