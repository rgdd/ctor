...
Our proposal extends Tor's consensus document to list CTRs, recognized CT logs,
and other necessary parameters that can be tuned for CTor.
...
Sections~\ref{sec:base:phase2}--\ref{sec:base:phase3}.  For now, assume that a
relay qualifies as a CTR if it is flagged as \texttt{stable} and not
\texttt{exit}, to spare relatively sparse on exit bandwidth and only use relays
that can be expected to stay online. Section~\ref{sec:privacy} discusses
...
the same logs as Tor Browser, such overlap is not required because the base
